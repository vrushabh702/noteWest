...
function Pare({ setStatement, setDurations, setStatusCollapse, setDateOfServices, data, URL, setAllData, viewEdit, setViewEdit, editMode, notes_id, commonData, saveAfterEdit,setpayloadsData,dataChildToParent }) {
    ...
    const postStatement = useCallback(debounce(postData, 200), [viewEdit]);
    ...
    async function postData(newData, data, id) {
        ...
        const postRequestData = {
            ...
            date_of_service: dataChildToParent ?  dataChildToParent : commonData.commonData.date_of_service,
        ...
        if (postdata) {
            ...
            setViewEdit(postdata.client_document_id)
            if (JsonData.clickCount < 1) {
                setJsonData({ ...JsonData, clickCount: 1 })
            }
            setJsonData({ ...JsonData, isLoading: false });
        }
    }
    ...
    const checkSelectedKeys = (keys, checked) => {
        ...
        // console.log(document.getElementById(keys[0]))
    ...
    React.useEffect(() => {
        setJsonData({ ...JsonData, data: data });
    }, [data])
        ...
        <form ref={CheckForm}>
            ...
            }
            {
                JsonData.data.map((e, i) => (
                    <changeFuns.Provider value={{ checkedFun, editMode, userId, tmpAllAddNewData, checkSelectedKeys, ...commonData }}>
